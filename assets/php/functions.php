...
function followUser($user_id){
    ...
    createNotification($cu['id'],$user_id,"got you on their Inspiration!");
    return mysqli_query($db,$query);
...
function like($post_id){
   ...
   if($poster_id!=$current_user){
    createNotification($current_user,$poster_id,"thinks you got this post on Fire!",$post_id);
   }
...
function addComment($post_id,$comment){
 ...
 $comment = mysqli_real_escape_string($db,$comment);
    ...
    if($poster_id!=$current_user){
        createNotification($current_user,$poster_id,"beamed-down thoughts on your Firework!",$post_id);
    }
...
function unlike($post_id){
    ...
    $poster_id = getPosterId($post_id);
    if($poster_id!=$current_user){
        createNotification($current_user,$poster_id,"un-fires your post!",$post_id);
    }
...
function addToTopTalk($post_id) {
    global $db;
  
    // Current user ID from session
    $current_user = $_SESSION['userdata']['id'];
  
    // Get post details (including poster ID)
    $post_details = getPosterId($post_id); // Replace with your function to fetch details
  
    // Check if post details exist
    if (!isset($post_details) || !isset($post_details['user_id'])) {
      return false; // Handle invalid post ID
    }
    ...
    $data = [
      ...
      'user_id' => $poster_id,
      'post_id' => $post_id,
      'comment' => '', // Add comment field if applicable (optional)
    ];
  
    // Build insert query
    $columns = implode(', ', array_keys($data)); // Get column names
    $values = "'" . implode("', '", array_values($data)) . "'"; // Prepare quoted values
  
    $query = "INSERT INTO topcom ($columns) VALUES ($values)";
  
    // Execute insert query
    $result = mysqli_query($db, $query);
  
    return $result;  // Return true/false for success/failure
  }
...
function unfollowUser($user_id){
    ...
    createNotification($current_user,$user_id,"cleared you out of their inspiration!");
    return mysqli_query($db,$query);
...
$response['status']=true;
    ...
    if(!$form_data['password']){
        $response['msg']="Uh oh, seems your Gatekeeper missing!";
        $response['status']=false;
        $response['field']='password';
    }
   
    if(!$form_data['username']){
        $response['msg']="Uh oh, seems your Avatar ID is missing!";
        $response['status']=false;
        $response['field']='username';
    }
    
    if(!$form_data['email']){
        $response['msg']="Uh oh, seems your Digital Mailbox is missing!";
        $response['status']=false;
        $response['field']='email';
    }
    
    if(!$form_data['last_name']){
        $response['msg']="Uh oh, seems your Lastrunner Name is missing!";
        $response['status']=false;
        $response['field']='last_name';
    }
    if(!$form_data['first_name']){
        $response['msg']="Uh oh, seems your Frontrunner Name is missing!";
        $response['status']=false;
        $response['field']='first_name';
    }
    if(isEmailRegistered($form_data['email'])){
        $response['msg']="Uh oh, seems this Digital Mailbox is already taken!";
        $response['status']=false;
        $response['field']='email';
    }
    if(isUsernameRegistered($form_data['username'])){
        $response['msg']="Uh oh, seems this Avatar-ID is already taken!";
        $response['status']=false;
        $response['field']='username';
    }
...
function validateLoginForm($form_data){
    ...
    $blank=false;
        ...
        if(!$form_data['password']){
            $response['msg']="Gatekeeper missing!";
            $response['status']=false;
            $response['field']='password';
            $blank=true;
        }
        ...
        if(!$form_data['username_email']){
            $response['msg']="Digital Mailbox/ Avatar-ID missing!";
            $response['status']=false;
        ...
        if(!$blank && !checkUser($form_data)['status'] ){
            $response['msg']="Something is Fuzzy, we couldn't find you!";
            $response['status']=false;
...
function validatePostImage($image_data){
    ...
    $response['status']=true;
        ...
        if(!$image_data['name']){
            $response['msg']="Please select an image to add a Firework";
            $response['status']=false;
       ...
       if($image_data['name']){
        ...
        if($size>2000){
            $response['msg']="Upload image that is of the size less than 1 Mb";
            $response['status']=false;
            $response['field']='post_img';
        }
       }
