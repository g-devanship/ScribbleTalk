...
if(isset($_SESSION['forgot_code']) && !isset($_SESSION['auth_temp'])){
    ...
    $action= 'forgotpassword';
            ...
            <form method="post" action="assets/php/actions.php?<?=$action?>">
                ...
                <h1 class="h5 mb-3 fw-normal">Forgot Your Gatekeeper ?</h1>
<?php
if($action=='forgotpassword'){
    ?>
  <div class="form-floating">
                    <input type="email" name="email" class="form-control rounded-0" placeholder="username/email">
                    <label for="floatingInput">Fuel login with Digital Mailbox :</label>
                </div>
                <?=showError('email')?>

<br>
                <button class="btn btn-primary" type="submit">Unleash the login guardian(code)!</button>
...
if($action=='verifycode'){
    ?>
<p>Blast in the verification beam  - <?=$_SESSION['forgot_email']?></p>
                ...
                <br>
                <button class="btn btn-primary" type="submit">Unleash the truth!</button>
