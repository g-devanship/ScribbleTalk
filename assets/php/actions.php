...
if(isset($_GET['block'])){
    ...
    $user = $_GET['username']; 
      if(blockUser($user_id)){
          ...
          echo "Uh oh, seems something is Fuzzy!";
      }
...
if($response['status']){
    ...
    header('location:../../?login&newuser');
        ...
        echo "<script>alert('Uh oh, seems something is Fuzzy!')</script>";
    }
...
if(isset($_GET['login'])){
    ...
    if($response['status']){
     ...
     sendCode($response['user']['email'],'Verify your Digital Mailbox',$code);
     }
    ...
    if(isset($_GET['resend_code'])){
            ...
            $_SESSION['code']=$code = rand(111111,999999);
            sendCode($_SESSION['userdata']['email'],'Verify your Digital Mailbox',$code);
    ...
    if(isset($_GET['verify_email'])){
       ...
       if(verifyEmail($_SESSION['userdata']['email'])){
        header('location:../../');
           ...
           $response['msg']='Encorrect Verfication Beam!';
           if(!$_POST['code']){
            $response['msg']='Enter 6 Count Digital Guardian !';
...
if(isset($_GET['forgotpassword'])){
    if(!$_POST['email']){
        $response['msg']="Put down your Digital Mailbox!";
        $response['field']='email';
        $_SESSION['error']=$response;
        header('location:../../?forgotpassword');

    }elseif(!isEmailRegistered($_POST['email'])){
        $response['msg']="Digital Mailbox is not registered";
        $response['field']='email';
        $_SESSION['error']=$response;
        header('location:../../?forgotpassword');
          ...
          $_SESSION['forgot_email']=$_POST['email'];
           $_SESSION['forgot_code']=$code = rand(111111,999999);
            sendCode($_POST['email'],'Forgot Your Gatekeeper ?',$code);
...
if(isset($_GET['verifycode'])){
    ...
    $_SESSION['auth_temp']=true;
     header('location:../../?forgotpassword');
        ...
        $response['msg']='Incorrect Verification Beam!';
        if(!$_POST['code']){
         $response['msg']='Enter 6 Count Guardian Code!';
...
if(isset($_GET['changepassword'])){
    if(!$_POST['password']){
        $response['msg']="Enter new Gatekeeper";
