...
if($user['id']!=$profile['id'] && !checkBS($profile['id'])){
  ...
  <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#chatbox" onclick="popchat(<?=$profile['id']?>)"><i class="bi bi-chat-fill"></i> Ping! </a></li>
                                <li><a class="dropdown-item " href="assets/php/actions.php?block=<?=$profile['id']?>&username=<?=$profile['username']?>"><i class="bi bi-x-circle-fill"></i> Cut-off!</a></li>
                            </ul>
                        </div>
    <?php
}
...
<a class="btn btn-sm btn-success"><i class="bi bi-file-post-fill"></i> <?=count($profile_post)?> Fireworks</a>
<a class="btn btn-sm btn-success" <?=count($profile['followers'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#follower_list"><i class="bi bi-people-fill"></i> <?=count($profile['followers'])?> Audience</a>
<a class="btn btn-sm btn-success" <?=count($profile['following'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#following_list"><i class="bi bi-person-fill"></i> <?=count($profile['following'])?> Inspirations</a>
...
?> 
<button class="btn btn-sm btn-danger unblockbtn" data-user-id='<?=$profile['id']?>' >Beam Back-in!</button>

<?php
}else if(checkBlockStatus($profile['id'],$user['id'])){ ?>
    <div class="alert alert-danger" role="alert">
    <i class="bi bi-x-octagon-fill"></i> @<?=$profile['username']?> Cut you off!
</div>
   <?php }else if(checkFollowStatus($profile['id'])){
   ?>
<button class="btn btn-sm btn-danger unfollowbtn" data-user-id='<?=$profile['id']?>' >Fizzle</button>
   
   <?php
}else{
    ?>
<button class="btn btn-sm btn-primary followbtn" data-user-id='<?=$profile['id']?>' >Embrace</button>
...
?>
        ...
        <h3 class="border-bottom">Fireworks</h3>
        <?php
...
if(checkBS($profile['id'])){
 ...
 <div class="alert alert-secondary text-center" role="alert">
    <i class="bi bi-x-octagon-fill"></i> Shhh...top secret content!
</div>
   <?php
...
Wanna drop your Thoughts? Embrace the Inspirer to roll! </div>
...
<div class="modal fade" id="following_list" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Inspirations</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?php
